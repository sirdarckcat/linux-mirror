...
class LinuxMirror {
  ...
  public async load(commit: string) {
    ...
    const promises = [
      ...
      this.workers[5].db.query("SELECT 'id='||syzkaller syzkaller, `commit` FROM syzkaller WHERE (`commit` >= ? AND `commit` <= ? || 'g') OR `commit` IN (SELECT `commit` FROM fixes WHERE LENGTH(fixes)>=4 AND fixes >= substr(?, 1, 4) AND fixes <= ? || 'g' AND SUBSTR(fixes, 0, instr(fixes, ' ')) = SUBSTR(?, 0, instr(fixes, ' ')))", [commit, commit, commit, commit, commit]),
